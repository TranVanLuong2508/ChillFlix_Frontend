...
export default function Header() {
  ...
  const { logOutAction, setTokenToTestApi, isAuthenticated, isLoading } = useAuthStore();
  const { removeCommentRealtime, createCommentRealtime, replyCommentRealtime, countCommentsRealtime } = useCommentStore();
  useEffect(() => {
    fetchGenresList();
    fetchCountriesList();
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on("replyComment", (data) => {
      ...
      setNotifications(prev => [
        { id: crypto.randomUUID(), type: "reply", message, createdAt: new Date() },
        ...prev,
      ]);
                ...
                {!isAuthenticated ? (
                  <button
                    ...
                    className="
                      ...
                      shadow-[0_0_10px_rgba(245,213,71,0.35)]
                      border border-[#d4af37]/50
                      transition-all duration-300 cursor-pointer
                      hover:shadow-[0_0_22px_rgba(245,213,71,0.75)]
